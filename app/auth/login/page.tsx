  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#F7F7F7', fontFamily: 'system-ui, sans-serif', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: '#FF4B2B', padding: '56px 24px 32px' }}>
        ...
        <div style={{ fontSize: '22px', fontWeight: 700, color: '#fff', letterSpacing: '-.5px', lineHeight: 1.2 }}>
      ...
      <div style={{ flex: 1, padding: '28px 20px' }}>
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div>
            <div style={{ fontSize: '12px', fontWeight: 600, color: '#888', marginBottom: '6px' }}>EMAIL</div>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="tu@email.com" required style={inputStyle} />
          </div>

          <div>
            <div style={{ fontSize: '12px', fontWeight: 600, color: '#888', marginBottom: '6px' }}>CONTRASENA</div>
          ...
          <div style={{ textAlign: 'center', fontSize: '13px', color: '#999', marginTop: '8px' }}>
            No tienes cuenta?{' '}
            <span onClick={() => router.push('/auth/register')} style={{ color: '#FF4B2B', fontWeight: 700, cursor: 'pointer' }}>
              Registrate
            </span>
          </div>
